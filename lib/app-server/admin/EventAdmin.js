    ...
    listPartners(function(error, partners)
    {
      ...
      else
      {
        response.render('admin/events/details.handlebars',
        {
          ...
          feet: "<script src='/vendor/jquery-tidy-table/jquery.tidy.table.min.js'></script><script src='/views/admin/events/details.js'></script><script src='/vendor/CSVParser.js'></script>",
...
EventAdmin.createEvent = function(request, response)
{
  var event = request.params.event;
  var eventBucket = event + '--event';
  eventBucket = eventBucket.toLowerCase();
  eventBucket = eventBucket.replace(/ /g, '');
  eventBucket = eventBucket.replace(/\(/g, '');
  eventBucket = eventBucket.replace(/\)/g, '');
  ...
  listEventBuckets(function(error, eventBuckets)
  {
    ...
    if (eventBuckets.indexOf(eventBucket) !== -1)
      return response.send(400, 'Event already exists');
    ...
    RiakDBAccessor.findOne('events', 'MetaData', function(error, metaData)
    {
      ...
      metaData[eventBucket] = { displayName: event };
...
EventAdmin.deleteEvent = function(request, response)
{
  ...
  RiakDBAccessor.deleteBucket(eventBucket, function()
  {
    if (error)
      return response.send(400, error);
  ...
  function update()
  {
    RiakDBAccessor.findOne(eventBucket, 'DataKeys', function(error, dataKeys)
    {
      ...
      RiakDBAccessor.update(eventBucket, 'DataKeys', dataKeys, function(error, dataKeys)
      {
        ...
        var propertyVal;
        if (property === 'Config' && _.isEmpty(request.body))
          propertyVal = DEFAULT_EVENT_CONFIG;
        else
          propertyVal = request.body;

        RiakDBAccessor.update(eventBucket, property, propertyVal, function(error, result)
        {
          if (error)
            return response.send(400, error);
