...
UserAdmin.read = function(request, response, next)
{
  request.app.db.models.User.findById(request.params.id).populate('roles.admin', 'name.full').populate('roles.account', 'name.full').exec(function(error, user)
  {
    ...
    SocialAccountConnector.fetchSocialAccounts(user, function(error, socialAccounts)
    {
      ...
      userObj.socialAccounts = socialAccounts;

      if (request.xhr)
        response.send(userObj);
      else
        response.render('admin/users/details', { data: { record: JSON.stringify(userObj) } });
    } );
  });
};
 ...
 *   request, response, next:
 */
UserAdmin.populateUser = function(userId, callback)
{
  App.db.models.User.findById(userId).populate('roles.admin', 'name.full').populate('roles.account', 'name.full').exec(function(error, user)
  {
    if (error)
...
UserAdmin.disconnectSocial = function(request, response, next)
{
  ...
  function patchUser()
  {
    request.app.db.models.User.findById(request.params.id, function(error, user)
    {
      if (error)
        return workflow.emit('exception', error);
      ...
      SocialAccountConnector.removeSocialAccount(user, request.params.socialType, function(error, user)
      {
        ...
        UserAdmin.populateUser(user.id, function(error, user)
        {
          if (error)
            return workflow.emit('exception', error);
