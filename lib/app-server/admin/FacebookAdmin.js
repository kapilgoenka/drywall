 ...
 * read
 *******************************************************************************
 * Fetch a Facebook record.
 *
 * Inputs:
 *   request, response, next:
 */
FacebookAdmin.read = function(request, response, next)
{
  request.app.db.models.FacebookAccount.findById(request.params.id).exec(function(error, facebookAccount)
  {
    if (error)
      return next(error);

    if (request.xhr)
      response.send(facebookAccount);
    else
      response.render('admin/facebook/details', { data: { record: JSON.stringify(facebookAccount) } });
  });
};
