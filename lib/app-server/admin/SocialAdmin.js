...
sa.SocialAdmin = (function()
{
  ...
  SocialAdmin.prototype.find = function(request, response, next)
  {
    ...
    RiakDBAccessor.listDocumentsInBucket(this.dbSchema, function(error, results)
    {
      if (error)
        return next(error);
  ...
  SocialAdmin.prototype.read = function(request, response, next)
  {
    ...
    RiakDBAccessor.findOne(this.dbSchema, { profileId: request.params.id }, function(error, socialAccount)
    {
      if (error)
        return next(error);
