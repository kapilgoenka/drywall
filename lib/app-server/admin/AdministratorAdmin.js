...
AdministratorAdmin.find = function(request, response, next)
{
  ...
  App.db.fetchAllDocsInBucket('admins', function(error, results)
  {
    if (error)
      return next(error);
...
AdministratorAdmin.read = function(request, response, next)
{
  ...
  {
    App.db.fetchAllDocsInBucket('admin_groups', function(error, adminGroups)
    {
      if (error)
  ...
  {
    App.db.findOne('admins', { _id: request.params.id }, function(error, admin)
    {
      if (error)
        return callback(error, null);
...
AdministratorAdmin.create = function(request, response, next)
{
  ...
  function createAdministrator()
  {
    ...
    App.db.update('admins', fieldsToSet._id, fieldsToSet, function(error, admin)
    {
      if (error)
...
AdministratorAdmin.update = function(request, response, next)
{
  ...
  function patchAdministrator()
  {
    ...
    App.db.findAndUpdate('admins', { _id: request.params.id }, fieldsToSet, function(error, admin)
    {
      if (error)
        return workflow.emit('exception', error);
...
AdministratorAdmin.linkUser = function(request, response, next)
{
  ...
  {
    App.db.findOne('admins', { userId: workflow.user._id }, function(error, admin)
    {
      if (error)
        return workflow.emit('exception', error);
  ...
  function patchUser()
  {
    App.db.findOne('users', { _id: workflow.user._id }, function(error, user)
    {
      if (error)
        return workflow.emit('exception', error);
...
AdministratorAdmin.deleteAdmin = function(request, response, next)
{
  ...
  {
    App.db.remove('admins', request.params.id, function(error)
    {
      if (error)
        return workflow.emit('exception', error);
