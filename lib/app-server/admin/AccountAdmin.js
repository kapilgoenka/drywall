...
AccountAdmin.find = function(request, response, next)
{
  ...
  function getResults(callback)
  {
    ...
    // {
    //   filters: filters,
    //   keys: 'name company phone zip createdBy',
    ...
    App.db.findAll('accounts', function(error, matches)
    {
      if (error)
        return callback(error, null);
...
AccountAdmin.read = function(request, response, next)
{
  ...
  {
    App.db.findOne(request.params.id, key, function(error, doc)
    {
      if (error)
        return callback(error, null);
  ...
  function asyncFinally(error, results)
  {
    ...
    {
      response.render('admin/accounts/details', { data: { record: JSON.stringify(outcome, null, 4) } });
    }
  }
...
AccountAdmin.create = function(request, response, next)
{
  ...
  function createAccount()
  {
    ...
    App.db.update('accounts', fieldsToSet._id, fieldsToSet, function(error, account)
    {
      if (error)
...
AccountAdmin.update = function(request, response, next)
{
  ...
  function patchAccount()
  {
    ...
    App.db.findAndUpdate('accounts', { _id: request.params.id }, fieldsToSet, function(error, account)
    {
      if (error)
        return workflow.emit('exception', error);
...
AccountAdmin.linkUser = function(request, response, next)
{
  function validate()
  {
    App.db.findOne('admins', { _id: request.user.accounts.admin }, function(error, admin)
    {
  ...
  function verifyUser()
  {
    App.db.findOne('users', { username: request.body.newUsername }, function(error, user)
    {
      if (error)
        return workflow.emit('exception', error);
  ...
  {
    App.db.findOne('accounts', { userId: workflow.user._id }, function(error, account)
    {
      if (error)
        return workflow.emit('exception', error);
  ...
  function patchUser()
  {
    App.db.findAndUpdate('users', { _id: workflow.user._id }, { accounts: { account: request.params.id } }, function(error, user)
    {
      if (error)
        return workflow.emit('exception', error);
  ...
  {
    App.db.findAndUpdate('accounts', { _id: request.params.id }, { user: { id: workflow.user._id, name: workflow.user.username } }, function(error, account)
    {
      if (error)
        return workflow.emit('exception', error);
...
AccountAdmin.unlinkUser = function(request, response, next)
{
  ...
  function patchAccount()
  {
    App.db.findOne('accounts', { _id: request.params.id }, function(error, account)
    {
      if (error)
       return workflow.emit('exception', error);
  ...
  {
    App.db.findOne('users', { _id: id }, function(error, user)
    {
      if (error)
        return workflow.emit('exception', error);
...
AccountAdmin.deleteAccount = function(request, response, next)
{
  ...
  {
    App.db.remove('accounts', { _id: request.params.id}, function(error, account)
    {
      if (error)
        return workflow.emit('exception', error);
