...
GroupAdmin.find = function(request, response, next)
{
  ...
  App.db.fetchAllDocsInBucket('admin_groups', function(error, matches)
  {
    if (error)
      return next(error);
...
{
  App.db.findOne('admin_groups', { _id: request.params.id }, function(error, adminGroup)
  {
    if (error)
      return next(error);
...
GroupAdmin.create = function(request, response, next)
{
  ...
  {
    App.db.findOne('admin_groups', { _id: SSUtil.slugify(request.body.name) }, function(error, adminGroup)
    {
      if (error)
        return workflow.emit('exception', error);
  ...
  function createAdminGroup()
  {
    ...
    App.db.update('admin_groups', fieldsToSet._id, fieldsToSet, function(error, adminGroup)
    {
      if (error)
        return workflow.emit('exception', error);
...
GroupAdmin.update = function(request, response, next)
{
  ...
  function patchAdminGroup()
  {
    ...
    App.db.findAndUpdate('admin_groups', { _id: request.params.id }, fieldsToSet, function(error, adminGroup)
    {
      if (error)
        return workflow.emit('exception', error);
...
GroupAdmin.deleteGroup = function(request, response, next)
{
  ...
  {
    App.db.remove('admin_groups', request.params.id, function(error)
    {
      if (error)
        return workflow.emit('exception', error);
