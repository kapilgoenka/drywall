...
GroupAdmin.create = function(request, response, next)
{
  ...
  {
    request.app.db.models.AdminGroup.findById(request.app.utility.slugify(request.body.name)).exec(function(error, adminGroup)
    {
      if (error)
  ...
  function createAdminGroup()
  {
    var fieldsToSet =
    {
      _id: request.app.utility.slugify(request.body.name),
