...
SocialAccountConnector.addSocialAccount = function(userId, socialProfile, callback)
{
  ...
  App.db.update(schema, socialAccountData._id, socialAccountData, function(error, account)
  {
    if (error)
      return callback(error);

    App.db.findOne('users', userId, function(error, user)
    {
      if (error || !user)
        return callback(error);
      ...
      App.db.findOne('organizations', 'userMap', function(error, userMap)
      {
        if (userMap && userMap[user.social[socialType]])
          user.organization = userMap[user.social[socialType]];

        App.db.update('users', userId, user, callback);
      });
    });
  });
};
...
SocialAccountConnector.removeSocialAccount = function(user, socialType, callback)
{
  ...
  App.db.remove(schema, { profileId: socialId }, function(error)
  {
    return callback(error);
  });
};
...
{
  App.db.findOne('users', request.user._id, function(error, user)
  {
    if (error)
      return next(error);
...
SocialAccountConnector.fetchSocialAccount = function(user, socialType, callback)
{
  ...
  App.db.findOne(bucket, { profileId: socialId }, function(error, socialAccount)
  {
    if (error)
      return callback(error);
...
function renderSettings(request, response, next, oauthMessage)
{
  ...
  {
    App.db.findOne(request.accountBucket, request.user.accounts[request.appId], function(error, account)
    {
      if (error)
