...
UserUpdater.updateContactInfo = function(request, response, next)
{
  ...
  var workflow = new wf.Workflow({ request: request, response: response, name: 'Update Contact Info' });
...
UserUpdater.updateIdentity = function(request, response, next)
{
  ...
  var workflow = new wf.Workflow({ request: request, response: response, name: 'Update Identity' });
  workflow.on('validate', validate);
...
UserUpdater.updatePassword = function(request, response, next)
{
  ...
  var workflow = new wf.Workflow({ request: request, response: response, name: 'Update Password for: ' + request.user.username });
  workflow.on('validate', validate);
  workflow.on('patchUser', patchUser);
  workflow.emit('validate');
};
...
{
  RiakDBAccessor.find('users', { username: newUsername }, function(error, result)
  {
    if (error)
...
{
  RiakDBAccessor.find('users', { email: newEmail }, function(error, result)
  {
    if (error)
      return workflow.emit('exception', error);
...
{
  RiakDBAccessor.findAndUpdate('users', userId, fieldsToSet, function(error, user)
  {
    if (error)
      return workflow.emit('exception', error);
