...
{
  App.db.findOne('users', { email: newEmail }, function(error, user)
  {
    if (error)
...
SignupUtil.duplicateUsernameCheck = function(newUsername, workflow, nextStep)
{
  ...
  App.db.findOne('users', { username: newUsername }, function(error, user)
  {
    if (error)
      return workflow.emit('exception', error);
...
SignupUtil.ensureUniqueUsername = function(username, socialId, callback)
{
  ...
  App.db.findOne('users', { username: username }, function(error, user)
  {
    if (error)
...
SignupUtil.createUser = function(createUserParams, callback)
{
  ...
  App.db.update('users', userData._id, userData, function(error, user)
  {
    if (error)
      return callback(error);
...
SignupUtil.logUserIn = function(user, workflow, request)
{
  ...
  {
    workflow.outcome.authToken = jwt.encode({ _id: user._id }, App.get('JWT_TOKEN_SECRET'));
