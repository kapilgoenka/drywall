...
DataExport.getExportData = function(request, response)
{
  ...
  if ( exportCache[ username ] && moment().diff( exportCache[username].lastAccess, 'seconds' ) < cacheTimeInterval )
    return exportCache[ username ].exportData;
  ...
  RiakDBAccessor.findOne('users', { username: username } , function(error, user)
  {
    ...
    RiakDBAccessor.findOne(bucket, exportKey, function(error, result)
    {
      ...
      if ( appId === 'keyword_insights' )
        result = response.send( insightsDataConverter( result ) );

      exportCache[ username ] = { lastAccess: moment(), exportData: result };
