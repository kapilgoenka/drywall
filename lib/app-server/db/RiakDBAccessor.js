...
RiakDBAccessor.fetchAllBuckets = function(callback)
{
  riak.buckets(function(error, buckets)
  {
    callback(error, buckets);
  });
};

RiakDBAccessor.fetchAllMediaCurationAccounts = function(callback)
{
  this.fetchAllBuckets(function(error, buckets)
  {
    var media_curation_accounts = [];
    _(buckets).each(function(bucket)
    {
      if (bucket.indexOf('media_curation') !== -1)
        media_curation_accounts.push(bucket);
    });
    callback(error, media_curation_accounts);
  });
};
...
RiakDBAccessor.deleteBucket = function(bucketName, callback)
{
  ...
  {
    riak.remove(bucket, key, callback);
  }
