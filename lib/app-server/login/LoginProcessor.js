...
LoginProcessor.isLoggedIn = function(request, response)
{
  var profile = request.body;
  var queryParams = { facebookId: String(profile.id) };
  ...
  RiakDBAccessor.findOne('users', queryParams, function(error, user)
  {
    ...
    response.send({ success: user.isLoggedIn !== false });
  });
};
...
LoginProcessor.loginUsernamePassword = function(request, response)
{
  ...
  var workflow = new wf.Workflow({ request: request, response: response, name: 'Login Username/Password' });
      ...
      else
      {
        if (request.xhr)
        {
          ...
          RiakDBAccessor.update('users', user._id, user, function(error, user)
          {
            ...
            response.send({ success: true, authToken: jwt.encode({ _id: user._id }, request.app.get('JWT_TOKEN_SECRET')) });
          });
        }
        else
        {
...
LoginProcessor.loginSingly = function(request, response, next)
{
  request._passport.instance.authenticate('singly', function(error, user, info)
  {
    ...
    // request.RiakDBAccessor.models.User.findOne({ 'twitter.id_str': twitterProfile.id }, function(error, user)
    RiakDBAccessor.findOne('users', { 'twitter.id_str': twitterProfile.id }, function(error, user)
    {
      if (error)
        return next(error);
